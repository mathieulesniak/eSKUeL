...
class sql_handler extends simple_object implements db_layer {
	...
	var $field_types = array(
		'numeric' 	=> array('bit', 'tinyint', 'bool', 'smallint', 'mediumint', 'int', 'bigint', 'float', 'double', 'decimal'),
		'string' 	=> array('char', 'varchar', 'tinytext', 'text', 'mediumtext', 'longtext', 'tinyblob', 'blob', 'mediumblob', 'longblog', 'binary', 'varbinary', 'enum', 'set'),
	...
	function table_rename($db_old_name, $table_old_name, $db_new_name, $table_new_name) {
		$sql  = "ALTER TABLE `" . $db_old_name . "`.`" . $table_old_name . "`";
		$sql .= " RENAME `" . $db_new_name . "`.`" . $new_name . "`";
	...
	function table_empty($db_name, $table_name) {
		$sql  = "DELETE FROM `" . $db_name . "`.`" . $table_name . "`";
